...
def show_checkbox_template_status():
    ...
    if tpl.get('enabled'):
        with st.container(border=True):
            ...
            with c1:
                st.caption(f"✅ Page: {tpl['page_name']}")
                st.caption(f"✅ Variable: ${{{tpl['variable']}}}")
                st.caption(f"✅ Keyword: {tpl['keyword']}")
            with c2:
...
def append_checkbox_template_to_file(file_path):
    ...
    try:        
        ...
        st.session_state['checkbox_template'] = {'enabled': False}
        if s1 and s2: return True, "✅ Locators and checkbox template appended"
    ...
    with panel_grid[0]:
        ...
        with st.container(border=True):
            ...
            if ws_state.get('keywords'):
                ...
                with c1: st.metric("📊 Total Keywords", stats['total_keywords'])
                with c2: st.metric("📁 Categories", stats['total_categories'])
                with c3: st.metric("🧩 Others", stats['uncategorized'])
                st.markdown("---")

                cat_sort = [k for k in cats.keys() if k != "🧩 Others"]
                cat_sort = sorted(cat_sort, key=get_category_priority)
                if "🧩 Others" in cats: cat_sort.append("🧩 Others")
        ...
        with st.container(border=True):
            ...
            with st.expander("📄 Add from HTML", expanded=True):
                for i, page in enumerate(ws_state['html_pages']):
                    ...
                    with c3:
                        if st.button("✏️ Edit", key=f"studio_edit_html_{i}", use_container_width=True): ws_state['editing_html_index'] = i; st.rerun()
                    with c4:
                        if len(ws_state['html_pages']) > 1 and st.button("🗑️", key=f"studio_remove_html_page_{i}", width='stretch'): ws_state['html_pages'].pop(i); st.rerun()

                if st.button("➕ Add HTML Page", type="secondary", width='stretch'):
        ...
        with st.expander("#### 📝 Locator Staging Area", expanded=True):
            ...
            if file_locs:
                ...
                for fname, locs in by_file.items():
                    with st.expander(f"📄 **{fname}** ({len(locs)} items)", expanded=False):
                        if st.button(f"🗑️ Unload locators from '{os.path.basename(fname)}'", key=f"unl_{fname}", type="secondary"):
